...
// --- Get Current Month and Year for display and database query ---
$displayMonthYear = date('Y-m');
$displayPeriodText = date('F Y');

// --- Fetch User's Budgets for the Current Month ---
$userBudgets = [];
$stmtBudgets = $conn->prepare('SELECT id, category, budget_limit FROM budgets WHERE user_id = ? AND month_year = ? ORDER BY category ASC');
if ($stmtBudgets) {
...
if (!$budgetsExist) {
    ...
    if ($stmtLastMonthBudgets) {
        ...
        // Re-fetch budgets to display the newly added ones
        if ($newBudgetsAdded) {
            $userBudgets = [];
            $stmtBudgets = $conn->prepare('SELECT id, category, budget_limit FROM budgets WHERE user_id = ? AND month_year = ? ORDER BY category ASC');
            if ($stmtBudgets) {
            ...
            $budgetsExist = true; // Set flag to true to show the cards
        }
    } else {
...
if ($budgetsExist) {
    ...
    $stmtSpending = $conn->prepare('SELECT category, SUM(amount) AS total_spend FROM transactions WHERE user_id = ? AND type = \'expense\' AND transaction_date BETWEEN ? AND ? GROUP BY category');
    if ($stmtSpending) {
    ...
    <div class='container my-5'>
        ...
        <div class='row g-4' id='budgetCardsContainer'>
            <?php foreach ($userBudgets as $budget):
                $currentSpend = $currentSpendingByCat[$budget['category']] ?? 0;
                ...
                <div class='card budget-card h-100 shadow-sm <?php echo $cardStatusClass; ?>'>
                    <div class='card-body'>
                        <div class='d-flex justify-content-between align-items-center mb-2'>
                            <h5 class='card-title mb-0'><?php echo htmlspecialchars($budget['category']); ?></h5>
                        ...
                        <div class='d-flex justify-content-between align-items-center mt-2'>
                            <small class='text-muted'>₹<?php echo $remainingText; ?> <?php echo $remainingDirection; ?></small>
                            <div>
                                <button class='btn btn-sm btn-light'
                                        ...
                                        data-month-year='<?php echo $displayMonthYear; ?>'>
          ...
          <div class='modal-body'>
            <form action='edit_budget_process.php' method='POST' id='editBudgetForm'>
              ...
              <div class='mb-3'>
                ...
                <input type='hidden' name='month_year' id='editBudgetMonthYear'>
          </div>
          <div class='modal-footer border-0'>
            <button type='button' class='btn btn-light' data-bs-dismiss='modal'>Cancel</button>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            editBudgetModal.addEventListener('show.bs.modal', function(event) {
                ...
                const monthYear = button.getAttribute('data-month-year');
                ...
                modalMonthYearInput.value = monthYear;
            });
