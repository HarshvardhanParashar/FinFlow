...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($transaction_id) || empty($description) || empty($amount) || empty($type) || empty($category) || empty($transaction_date)) {
    ...
    $stmt = $conn->prepare("UPDATE transactions SET description = ?, amount = ?, type = ?, category = ?, transaction_date = ? WHERE id = ? AND user_id = ?");
    ...
    // CRITICAL FIX: The type string 'sdsissi' is correct, but let's confirm the order.
    // 's' for description (string)
    // 'd' for amount (double/decimal)
    // 's' for type (string)
    // 's' for category (string)
    // 's' for transaction_date (string)
    // 'i' for id (integer)
    // 'i' for user_id (integer)
    $stmt->bind_param("sdsissi", $description, $amount, $type, $category, $transaction_date, $transaction_id, $user_id);
